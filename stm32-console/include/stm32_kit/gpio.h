...
static uint8_t STM32_port_name[] = "ABCDEFGHI";
 ...
 */
void io_set(enum pin pin, int value);
 ...
 * @return Hodnota pinu
 */
int io_read(enum pin pin);

void GPIO_clock_enable(enum pin pin);
void GPIO_clock_disable(enum pin pin);
//uint32_t io_port_source(GPIO_TypeDef *port);
int io_pin(enum pin pin);
uint32_t io_pin_pos(enum pin pin);
uint32_t io_port_offset(enum pin pin);
//GPIO_TypeDef* io_port(enum pin pin);
